...
#define REFRESH_COLOR_LIMIT_L_REL_AMP		0.95
#define REFRESH_BLEACH_LIMIT_H_REL_AMP 		0.3
#define REFRESH_BLEACH_LIMIT_L_REL_AMP		0.5

#define REFRESH_COLORING_VOLTAGE 			1.3
#define REFRESH_BLEACHING_VOLTAGE 			0.7

#define COLORING_VOLTAGE 					1.3			// V - Absolute value for Color Pulse Voltage
#define COLORING_TIME 						350			// ms - Color time
#define BLEACHING_VOLTAGE 					0.7			// V - Absolute value for Bleach Pulse Voltage
#define BLEACHING_TIME 						350			// ms - Bleach time
...
class YNV_ECD
{
	...
	private:
		...
		bool m_bleachRequiredFlag;							// Use this flag to indicate that bleaching is required
		bool m_colorRequiredFlag;							// Use this flag to indicate that coloring is required
		bool m_refresh_color_needed;						// Flag to enable refresh colored segments routine
  		bool m_refresh_bleach_needed;						// Flag to enable refresh bleached segments routine

		//FUNCTIONS
		void execute_bleach(void);
		void execute_color(void);
		void updateRefreshLimits(void);
		void disableAllSegments(void);
};
